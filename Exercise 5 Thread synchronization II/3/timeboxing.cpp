...
void *car_thread(void *arg) {
   int car_id = *(int*)arg;
    while (true) {
        ...
        auto end_time = chrono::steady_clock::now(); // end the timer
        auto elapsed_time = chrono::duration_cast<chrono::milliseconds>(end_time - start_time); // calculate the elapsed time in milliseconds
        if (elapsed_time.count() > 5000) { // if elapsed time is more than 5 seconds, exit the thread
            cout << "car " << car_id << " exceeded time limit and is leaving" << endl;
            pthread_exit(NULL);}}}
int main() {
    ...
    for (size_t i = 1; i <= 10; i++) {       // Create 10 car threads
